...
export const topics: DocsTopic[] = [
  {
    ...
    render: (ctx) => (
      <DocsProse>
        ...
        <p>
          Four fields at the top level, two of them required. The keys are one letter because the
        ...
        <ul>
          ...
          <li>
            <code>d</code> &mdash; <strong>required.</strong> The distinct id: an anonymous id you
            generate once and persist. See below.
          </li>
          <li>
            <code>r</code> &mdash; the resource. Attributes true of the whole client rather than of
            one event, such as <code>service.version</code> and <code>os.type</code>. They are
            merged <em>under</em> each event&rsquo;s own attributes, so an event that sets the same
            key wins.
          </li>
          <li>
        ...
        <h2>The distinct id</h2>

        <p>
          Required on every batch, generated by you, and persisted to{" "}
          <strong>machine-local</strong> storage. It identifies an installation, not a person: on
          Windows that means <code>%LOCALAPPDATA%</code> and never <code>%APPDATA%</code>, because
          a roaming profile syncs between machines and one person on three of them would report as
          one install instead of three.
        </p>

        <p>
          It is anonymous and it is scoped to this one source. Nothing here is ever joined to an id
          from another source. To count one person once across two of them, send{" "}
          <code>user.id</code> in <code>r</code> with the same value on both.
        </p>

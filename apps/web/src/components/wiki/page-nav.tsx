...
import { sectionedTopics, topicTitle, type WikiTopic } from "./registry.js";
 ...
 *
 * The pair is not filtered by the picked source. Nothing in the wiki is any
 * more; see `sectionedTopics`.
 */

function neighbours(slug: string): { previous: WikiTopic | null; next: WikiTopic | null } {
    ...
    <Show when={pair().previous || pair().next}>
      ...
      >
        {/* Always both cells, even when one is empty, so a page at either end
            of the wiki does not pull its one link into the middle. */}
        <div class="min-w-0 flex-1">
          <Show when={pair().previous}>
            {(topic) => (
              <NavLink
                slug={topic().slug}
                label={i18n.t("wiki.previous")}

...
export function hashIp(salt: string, projectId: string, ip: string): string {
  return createHash("sha256").update(salt).update("\0").update(projectId).update("\0").update(ip).digest("hex");
}
...
export async function estimateFirstRun(
  ...
  const candidates = ctx.repos.downloadHints.candidates(
    args.projectId,
    ipHash,
    args.os,
    args.at - ctx.config.estimateWindowMs,
    args.at
  );
  ...
  const to: Distinct = { type: "web_visitor", id: best.web_visitor_id };
  ...
  return {
    ...
    web_visitor_id: best.web_visitor_id,
  };
}

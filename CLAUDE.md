...
### 3. Four columns are promoted. Everything else lives in `attributes`.

- The promoted columns are `project_id`, `time`, `severity`, `name`. That is the whole list.
  `ingested_at` is stamped beside them for debugging and is not a dimension.
- `os`, `app_version`, `url`, `referrer`, the utm fields and **all three identity keys**
  (`user.id`, `device.id`, `session.id`) are **attributes**, queried by path, not columns. So are
  OTel's `body`, `trace_id` and `span_id`: the last two are reserved and currently unused.
- **No identity is promoted and none is required.** There was a `distinct_id` column, NOT NULL,
  that every client had to invent a value for. It is gone: deleted, not deprecated. It forced an
  id to exist on platforms where no honest one does, so in a browser it was a storage key
  pretending to be a device and on a server it was a process pretending to be a person. Do not
  restore it and do not reintroduce it in another shape.
...
### 6. Identity is three optional attributes, per surface, never inferred.

Full detail in `docs/identity.md`. The rules that must not drift:

- **Three slots, three setters, three lifespans, and all three are optional.** `user(id)` sets
  `user.id` and lives until it is cleared; `device(id)` sets `device.id` and lives forever;
  `session(id)` sets `session.id` and lives until it is replaced. There is no fourth id under
  them holding anything up, and **an entry carrying none of the three is a legal entry**. On a
  server it is the ordinary one.
- **Absent means we do not know, and that is an answer rather than a hole.** A client fills a slot
  in for itself only where the platform genuinely knows: a desktop install IS a machine, so
  `.NET` and `tauri` write `device.id` from the id they persist. A browser is NOT a machine, so
  the tag writes no device at all unless the customer switched fingerprinting on AND the visitor
  consented. A server client (`node`, `go`, `python`) fills in **nothing** by default: a process
  is not a person and a request is not a visit, so the developer decides what to measure.
- `user.id` is **only ever the string the customer passed to `user()`**. We never invent one,
...
- A unique has exactly one definition: `count(distinct coalesce(attributes ->> 'user.id',
  attributes ->> 'device.id', attributes ->> 'session.id'))`, scoped within one surface. Best
  available identity wins, and an entry with none of the three is counted in no unique at all.
- **Identity travels as ONE UNIT, from ONE layer.** Every client that has layers (a per-call
  identity, an ambient request scope, a client-level default) takes all three from the innermost
  layer that states any of them, and does not consult the layers below it for the other two.
  Three independent fallback chains is the bug: a background job recorded inside a request would
  name its own device and keep the requester's `user.id`, and the coalesce above then counts that
  job as that customer.
- **Naming a different person cuts the session.** A sign-in is a boundary and one session
  spanning two accounts belongs to neither. Naming the same person again is a no-op and must
  stay one, because a router calls it on every route change.
- **`device.id` identifies an INSTALLATION, not a person, so a client that persists it uses
  ...
  one id and report as one install instead of three.
...
- **Two sources in one project are not linked to each other.** No inference, no probabilistic
  matching, no IP heuristics, no merging, ever. If a customer wants a person joined across
  surfaces they call `user()` with the same id on both. That is their data and their decision,
  not something we reconstruct from behaviour. There is no `session.previous_id` and no other
  breadcrumb tying one session to the one before it: rotating a session ends it.
- **The browser fingerprint is the one derived id, and it is gated twice.** `fingerprint: true`
  in the tag's config AND `consent(true)` from the visitor. It hashes a handful of stable-ish
  signals, it collides between identical machines, it changes when the OS updates, and the docs
  say so in those words. It is a trend line, never a headcount. Off by default, and nothing else
  anywhere derives an identity from anything.
- There is no server-side identity computation. The edge resolves a source key to a `project_id`
  and a source id, stamps `ingested_at`, and writes the row.
...
user(userId)              sets user.id from here on. Never inferred, never guessed
device(deviceId)          sets device.id. Only where a machine is honestly known
session(sessionId)        sets session.id. Rotating one is calling this again
flush()                   best effort, still bounded, still never throws
```

`user`, `device` and `session` are the three identity slots of rule 6 and are spelled the same
way in every client. There is no `identify()`, no `reset()` and no `newSession()`: signing out is
`user(null)`, and a new session is `session(<new id>)`.
...
The clients differ in what they fill in for themselves, and the platform is the whole reason.
`.NET` and `tauri` are the desktop clients: an install is a machine and a run is a visit, so they
persist a `device.id` and open a `session.id` on their own. `node`, `go` and `python` are server
clients and fill in nothing at all. The browser tag differs the other way again. It keeps its own
session (thirty idle minutes, or arrival from a new referring site), it has `page()`, `navigated()`
for a framework router that knows the route before `history` does, and `consent()`, and it has no
level helpers at all, because the byte budget is real and a helper nobody calls still costs every
visitor bytes.
...
run a second analytics tag alongside this one. It has a hard gzipped budget enforced by
`packages/web-tag/test/size.test.ts`, because it loads on someone else's marketing site. The
number lives in `packages/web-tag/build.ts` and is 4.75 KiB. It has moved twice, and the rule for
moving it is written there: displace something, or make an argument the size of the ones already
in that comment. It is not a high-water mark and it does not follow the size down.
...
packages/web-tag/   the browser tag, vanilla TS, esbuild, gzipped budget in build.ts
...
docs/               measured design references, identity, the delivery policy, and billing
  identity.md       the three identity slots, their lifespans, and what each client fills in
```

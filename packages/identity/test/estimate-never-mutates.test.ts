...
describe("an estimate edge never mutates a person id", () => {
  test("resolve is unchanged on both sides of the edge", async () => {
    ...
    const beforeInstall = await resolver.resolve(PROJECT, install("i1"));
    const beforeWeb = await resolver.resolve(PROJECT, web("v1"));

    await resolver.link(PROJECT, install("i1"), web("v1"), "estimate", 0.7);

    expect(await resolver.resolve(PROJECT, install("i1"))).toBe(beforeInstall);
    expect(await resolver.resolve(PROJECT, web("v1"))).toBe(beforeWeb);
    expect(beforeInstall).not.toBe(beforeWeb);
  });

  test("the two sides stay two separate people", async () => {
    const { resolver } = fresh();
    await resolver.link(PROJECT, install("i2"), web("v2"), "estimate", 0.7);
    expect(await resolver.resolve(PROJECT, install("i2"))).toBe(seedPersonId(PROJECT, install("i2")));
    expect(await resolver.resolve(PROJECT, web("v2"))).toBe(seedPersonId(PROJECT, web("v2")));
  });

  test("link() refuses to hand back a person for an estimate", async () => {
    const { resolver } = fresh();
    const result = await resolver.link(PROJECT, install("i3"), web("v3"), "estimate", 0.42);
  ...
  test("no override row is written", async () => {
    ...
    await resolver.link(PROJECT, install("i4"), web("v4"), "estimate", 0.7);
    expect(await store.pendingOverrides()).toEqual([]);
  });

  test("squash rewrites nothing, so stored events keep their person", async () => {
    const { store, resolver } = fresh();
    const installPerson = await resolver.resolve(PROJECT, install("i5"));
    const webPerson = await resolver.resolve(PROJECT, web("v5"));
    ...
    await resolver.link(PROJECT, install("i5"), web("v5"), "estimate", 0.9);
  ...
  test("an estimate cannot bridge two components that an exact edge would", async () => {
    ...
    await resolver.link(PROJECT, web("L1"), install("L2"), "token");
    await resolver.link(PROJECT, web("R1"), install("R2"), "token");
    await resolver.link(PROJECT, install("L2"), install("R2"), "estimate", 0.8);

    const left = await resolver.resolve(PROJECT, web("L1"));
    const right = await resolver.resolve(PROJECT, web("R1"));
    expect(left).not.toBe(right);
    expect(await resolver.resolve(PROJECT, install("L2"))).toBe(left);
    expect(await resolver.resolve(PROJECT, install("R2"))).toBe(right);
  });

  test("an estimate does not survive as a shortcut once an exact edge arrives elsewhere", async () => {
    const { resolver } = fresh();
    await resolver.link(PROJECT, install("i6"), web("v6"), "estimate", 0.8);
    await resolver.link(PROJECT, install("i6"), web("v7"), "token");

    const person = await resolver.resolve(PROJECT, install("i6"));
    expect(await resolver.resolve(PROJECT, web("v7"))).toBe(person);
    // v6 was only ever a guess. It stays out.
    expect(await resolver.resolve(PROJECT, web("v6"))).not.toBe(person);
  });

  test("the store never returns an estimate edge to traversal", async () => {
    const { store, resolver } = fresh();
    await resolver.link(PROJECT, install("i7"), web("v8"), "estimate", 0.6);
    const edges = await store.exactEdgesTouching(PROJECT, [install("i7"), web("v8")]);

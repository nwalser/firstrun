...
describe("squash", () => {
  test("drains overrides into events and deletes what it drained", async () => {
    ...
    const webPerson = await resolver.resolve(PROJECT, { type: "web_visitor", id: "v1" });
    const installPerson = await resolver.resolve(PROJECT, { type: "install", id: "i1" });
  ...
  test("running twice is a no-op the second time", async () => {
    ...
    store.events.push({
      project_id: PROJECT,
      event_id: "e1",
      person_id: await resolver.resolve(PROJECT, { type: "install", id: "i2" }),
      web_visitor_id: null,
      install_id: "i2",
      account_id: null,
    });
  ...
  test("queries are correct before squash runs, via the override", async () => {
    ...
    const overrides = await store.getOverrides(PROJECT, [
      { type: "install", id: "i3" },
      { type: "web_visitor", id: "v3" },
    ]);
    const resolved = new Set([
      overrides.get("install i3") ?? (await resolver.resolve(PROJECT, { type: "install", id: "i3" })),
      overrides.get("web_visitor v3") ??
        (await resolver.resolve(PROJECT, { type: "web_visitor", id: "v3" })),
    ]);

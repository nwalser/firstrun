 ...
 *
 * Four things are promoted and the rest is one JSON map: `project_id`, `time`,
 * `severity`, `name`. A closed set of columns is a closed set of questions, and
 * which question a customer needs is the one thing we cannot know in advance.
 * `body`, `trace_id` and `span_id` are therefore attributes like any other:
 * they are part of the spec's vocabulary, not part of ours, and promoting one
 * later is a generated column over `attributes` rather than a schema break.
 *
 * ## No identity is promoted, and none is required
 *
 * `user.id`, `device.id` and `session.id` are three OPTIONAL attributes, set by
 * the three client calls of the same names, and an entry may carry none of
 * them. That is not a gap to be filled in: it is the honest answer whenever the
 * developer has not told us, and a server logging an exception normally has not.
 *
 * There used to be a required `distinct_id` column here, generated by every
 * client for itself. It is gone, because it forced an id to exist on platforms
 * where no honest one does. In a browser it was a storage key pretending to be
 * a device; on a server it was a process pretending to be a person. Now a
 * client sets what it can actually know, and nothing invents the rest.
 */
 ...
 * one entry, such as `service.version`, `os.type`, and the three identity keys
 * `user.id`, `device.id` and `session.id`. They are merged UNDER each entry's
 * own attributes at the edge, so a row is self-contained and a query never has
 * to join to find out which app version something came from. An entry that sets
 * the same key wins, because the entry is the more specific statement.
 *
 * Identity rides here and nowhere else. There is no top-level id field: the
 * batch used to carry `d`, and a client that still sends one is not rejected,
 * it is ignored like any other key the schema does not name.
 */

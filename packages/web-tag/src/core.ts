...
export function createTag(env: Env, config: TagConfig) {
  ...
  if (env.get(KEY_CONSENT) === "1") {
    consented = true;
    distinctId = env.get(KEY_DID) || env.uuid();
    env.set(KEY_DID, distinctId);
   ...
   * `identify()`, and is absent until they do.
   */
   ...
   */
  function flush(): void {
    if (!consented || !distinctId || buffer.length === 0) return;
    const e = buffer;
    buffer = [];
    env.send(
      config.host + "/v1/e",
      JSON.stringify({ k: config.sourceKey, d: distinctId, r: resource(), e })
    );
  }
   ...
   * carrying one distinct id and would have to re-derive this same rule on every
   * read, from worse information, forever. Cutting once at the source makes a
   * session id a fact the rest of the system can just group by.
   */
  ...
  function saveSession(): void {
    if (consented && lastSeen) env.set(KEY_SESSION, lastSeen + "|" + lastRef);
  }
  ...
  /**
   * The customer's own id for this person, from `identify()`.
   *
   ...
   * matters re-calls `identify()` from the session the customer already has.
   */
  function identify(id?: string | null): void {
    userId = id ? String(id) : undefined;
  }
  ...
  return {
    call(cmd: string, a?: unknown, b?: unknown): unknown {
      ...
      if (cmd === "identify") return identify(a as string | null | undefined);
